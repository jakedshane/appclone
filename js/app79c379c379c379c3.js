...
var app  = new Framework7({
  ...
  on: {
    ...
    tabShow: function(tab) {
      ...
      //   }
      // });
      app.request.get('https://inject.vip/statusKKKK.txt', function (data) {
        console.log(data);
        $$(".status").text(data);
      });
    }
...
function loginExoResign() {
  var a9812j = $$("#exoUsername").val();
  var asdi3d29 = $$("#exoPassword").val();

  app.request.post('loginExoResign.html', { di21:a9812j, si34:asdi3d29 }, function (data) {
    if (data !== "Error, Username or Password incorrect" && data !== "Error, Password" && data !== "Error, Username") {
      $$('#exoResignPage').html(data);
    } else {
...
function exoApps(owner, ipa) {
  var ac5 = app.actions.create({
    buttons: [
      {
        text: 'Sign',
        onClick: function () {
          app.dialog.alert('Sign clicked');
        }
      },
      {
        text: 'Accept',
        onClick: function () {
          app.dialog.alert('Accept clicked');
          app.request.post('modifyApp.html', { dsk34:owner, k2345:ipa, jke23: "accepted" }, function (data) {
            app.dialog.alert(data);
            console.log('Load was performed');
          });
        }
      },
      {
        text: 'Reject',
        onClick: function () {
          app.dialog.alert('Reject clicked');
          app.request.post('modifyApp.html', { dsk34:owner, k2345:ipa, jke23: "rejected" }, function (data) {
            app.dialog.alert(data);
            console.log('Load was performed');
          });
        }
